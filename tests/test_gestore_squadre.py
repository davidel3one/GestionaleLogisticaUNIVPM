...
def test_elimina_squadra_definitivamente_ammessa_anche_se_ancora_attiva(session_factory):
    with session_factory() as session:
        _squadra_con_composizione(session, "SQ1")
        session.commit()
...
def test_riattiva_squadra(session_factory):
    with session_factory() as session:
        session.add(Squadra(id="SQ1", flg_attiva=False, data_creazione=datetime(2026, 1, 1)))
        session.commit()

    risultato = GestoreSquadre(session_factory).riattiva_squadra("SQ1")
...
def test_riattiva_squadra_gia_attiva_rifiutata(session_factory):
    with session_factory() as session:
        _squadra_con_composizione(session, "SQ1")
        session.commit()
...
def test_riattiva_squadra_inesistente_rifiutata(session_factory):
    risultato = GestoreSquadre(session_factory).riattiva_squadra("SQ-X")

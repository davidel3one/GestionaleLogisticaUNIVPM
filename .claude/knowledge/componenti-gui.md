...
- Padding inferiore del content: **8px imposto dal componente** (`CONTENT_PADDING_BOTTOM`). Era 0px fino al 2026-07-17: il mockup non lo specifica, ma con un footer/divider sotto il contenuto l'ultimo widget passato dal chiamante toccava la linea divisoria senza respiro (bug visivo reale, verificato su "Modifica esito consegna" - ultima prova allegata incollata alla linea) - corretto su richiesta esplicita dell'utente con un valore piccolo apposta (non quanto `CONTENT_PADDING_TOP=32`, solo per staccare visivamente l'ultimo elemento dalla linea).
...
**Valori esatti dal mockup**: popover sfondo `#EAEAEA`, radius 10px; testo Inter 12px/Medium `#2E2E2E`, padding 12px verticale / 16px orizzontale; icona trigger 18×18 (Lucide `info`, identificata per confronto strutturale dei path SVG, non a occhio); gap orizzontale 10px tra icona e popover, popover verticalmente centrato rispetto all'icona; nessuna freccia/puntatore verso l'icona (il popover nel mockup ha un solo figlio, il testo).

**Bordo 1px + ombra più marcata (2026-07-16, richiesta esplicita dell'utente, non nel mockup)**: il popover (`Popover.paintEvent`) aveva solo il fill arrotondato dipinto a mano (`NoPen`, nessun contorno) — ora un `QPen` 1px colore `POPOVER_TEXT_COLOR` (`#2E2E2E`, riusato) disegna anche il contorno, con il rettangolo inset di 0.5px per evitare che il tratto (centrato sul bordo) venga tagliato a metà dai limiti del widget. Reso "uniforme" perché `Popover` è un unico componente riusato sia da `Tooltip` (icona info) sia dall'hover di troncamento testo in `Table._ElidingLabel` — la modifica si applica automaticamente a entrambi. Ombra aumentata in coppia col bordo ("un po' di rialzo", scelta non misurata): `QGraphicsDropShadowEffect(color=#0000002E, offset=(0,12), blur=32)`, era `(color=#00000026, offset=(0,8), blur=24)`.
...
Componenti la cui forma è ritagliata esattamente sul layout di **una** pagina (non genuinamente riusabili altrove) vivono in `gui/pages/<pagina>/components/`, non in `gui/components/` — pattern deciso il 2026-07-15 costruendo la Dashboard. Stesso processo "Aggiungere un componente nuovo" sopra, stessa documentazione qui sotto, solo import path diverso.

**Spostamento sotto `gui/pages/` (2026-07-17)**: Dashboard, Autenticazione e Pianificazione vivevano come cartelle di primo livello sotto `gui/` (`gui/dashboard/`, `gui/autenticazione/`, `gui/pianificazione/`), a fianco di `gui/pages/` invece che dentro - inconsistente, dato che sono a tutti gli effetti pagine come Camion/Dipendenti/Ordini/Squadre/Viaggi. Spostate dentro `gui/pages/` su richiesta esplicita dell'utente (`gui/pages/dashboard/`, `gui/pages/autenticazione/`, `gui/pages/pianificazione/`), con import aggiornati ovunque nel codice e ri-esportate anche da `gui/pages/__init__.py` (`AutenticazionePage`, `DashboardPage`, `PianificazionePage`), stesso pattern delle altre 5 pagine.

**Dashboard** (`gui/pages/dashboard/components/`, import `from gestionale_logistica.gui.pages.dashboard.components import ActivityRow, KpiCard, PlanningDayCard`):
...
I dati reali (KPI aggregati, conteggio viaggi per i prossimi 7 giorni, feed attività) sono letti da `gui/pages/dashboard/dashboard_data.py` — nessun RF1-RF19 definisce una Dashboard, le query aggregano dai modelli esistenti (vedi docstring del file per le assunzioni dichiarate: definizione di "disponibile", formula dei trend, provenienza dei 4 tipi di evento nel feed attività).

**Pianificazione** (`gui/pages/pianificazione/components/`, import `from gestionale_logistica.gui.pages.pianificazione.components import AvvioCard, CompositionCard, PlanKpiCard, RigaOrdineComposizione, RigaOrdineSuggerito, SuggestionSection, CATEGORIA_BADGE_LABELS` — `DateFilterField` promosso a `gui/components/`, vedi `## DatePicker`, ma resta riesportato anche da qui per compatibilità):
...
- **`AvvioCard(parent=None)`** — card "Avvia composizione viaggio": `Select` senza label (placeholder "Seleziona squadra") + `DateFilterField` + bottone "Avvia composizione", messaggio di errore opzionale (`show_alert`/`hide_alert`, stesso stile di `CompositionCard`). API: `set_composizioni_disponibili(list[tuple[id, "Squadra: #N"]])`, `data_selezionata() -> date`. Segnali: `avviaRequested(str, date)`, `dataChanged(date)`. **"Squadra" invece di "Composizione"** nel testo visibile all'utente (placeholder, opzioni del menu, hint "N squadre attive disponibili...") — 2026-07-16, richiesta esplicita dell'utente: la terminologia interna resta `ComposizioneSquadra`/`composizione_id`, solo l'etichetta mostrata è cambiata, per coerenza con "Seleziona squadra" già usato altrove (es. Viaggi).

...
- **RF12/RF13 (motore di ottimizzazione) IMPLEMENTATE, REVISIONATE E STRESS-TESTATE il 2026-07-11** su branch `feat/ottimizzatore-rf12-rf13` (nessun commit ancora creato — da autorizzare esplicitamente). Prima versione (single-MILP con dispersione pesata) cancellata dopo review per un bug di calibrazione; redesign v2 (DBSCAN + knapsack per cluster + Held-Karp scritto a mano per un vincolo di durata/orario lavorativo, RNF4 alzato a 3 minuti) implementato, revisionato, sottoposto a stress test esaustivo (26 scenari) e ad audit avversariale indipendente — **92 test verdi**, un bug reale trovato e risolto (parametro `data` ridondante in `calcola_piano`, rimosso). Vedi `design-ottimizzatore.md`, sezione "Aggiornamento 2026-07-11", per i dettagli completi. **RF10/RF11** (composizione manuale viaggio, validazione realtime in GUI) restano da fare — l'idoneità categoria↔risorsa esiste già dentro l'ottimizzatore ma non è ancora richiamabile da un flusso manuale.

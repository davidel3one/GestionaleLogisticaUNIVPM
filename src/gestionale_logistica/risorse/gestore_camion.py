...
class GestoreCamion:
    ...
    def elimina_camion_definitivamente(self, id_: str) -> RisultatoOperazioneCamion:
        """Hard-delete, irreversibile: rimuove il camion dal database. Rifiuta se e' mai stato
        assegnato a una ComposizioneSquadra, attiva o storica - altrimenti romperebbe il vincolo di
        integrita' referenziale con quella riga (e lo storico viaggi che ci si aggancia)."""
        with self.session_factory() as session:
            mezzo = session.get(Camion, id_)
            if mezzo is None:
                return RisultatoOperazioneCamion(ok=False, motivo=f"Camion '{id_}' non trovato")
            ...
            if composizione_bloccante is not None:
                return RisultatoOperazioneCamion(
                    ok=False,
                    motivo="Impossibile eliminare definitivamente: il camion ha fatto parte di una squadra",
                )

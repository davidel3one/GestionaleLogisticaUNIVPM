...
class CamionPage(QWidget):
            ...
            [
                ...
                ColumnDef(
                    ...
                    actions=[
                        ...
                        RowAction(
                            "trash-2",
                            self._disattiva_riga,
                            predicate=lambda r: r["stato"] != STATO_DISMESSO,
                        ),
                        RowAction(
                            "rotate-ccw",
                            self._riattiva_riga,
                            predicate=lambda r: r["stato"] == STATO_DISMESSO,
                            tooltip="Rimetti in servizio",
                        ),
                    ],
                ),
            ]
    ...
    def _disattiva_riga(self, riga: dict) -> None:
        risultato = self._gestore.disattiva_camion(riga["id"])
        if not risultato.ok:
            QMessageBox.warning(self, "Impossibile dismettere", risultato.motivo or "Operazione rifiutata.")
        self._reload()

    def _riattiva_riga(self, riga: dict) -> None:
        risultato = self._gestore.riattiva_camion(riga["id"])
        if not risultato.ok:
            QMessageBox.warning(self, "Impossibile riattivare", risultato.motivo or "Operazione rifiutata.")
        self._reload()
    ...
    def _apri_modale_modifica(self, riga: dict) -> None:
        campo_tipo = Select("Tipo mezzo", options=self._opzioni_tipo_mezzo())
        campo_tipo.set_value(riga["tipo_mezzo"])
        campo_sponda = BooleanToggle("Sponda idraulica")
        campo_sponda.set_value(riga["flg_sponda_idraulica"])

        bottone_annulla = Button(ButtonVariant.SECONDARY, "Annulla")
        bottone_conferma = Button(ButtonVariant.PRIMARY, "Salva")
        modale = Modal("Modifica camion", width=560, footer_buttons=[bottone_annulla, bottone_conferma])
        modale.content_layout.addLayout(riga_2_colonne(campo_tipo, campo_sponda))

        bottone_annulla.clicked.connect(modale.close)

        def _conferma() -> None:
            risultato = self._gestore.modifica_camion(
                riga["id"],
                tipo_mezzo=campo_tipo.value(),
                flg_sponda_idraulica=campo_sponda.value(),
            )
